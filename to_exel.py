...
from schedule_generator.constants import (
    MONTH_TO_NUMBER,
    QUANTITY_OF_WORKING_HOURS, WEEKDAYS, XLSX_PATH, YEAR
    )
...
def add_summary_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Д1"] = df[day_cols].apply(
        lambda row: sum(1 for x in row if x == "Д1"), axis=1
        )
    df["Д2"] = df[day_cols].apply(
        lambda row: sum(1 for x in row if x == "Д2"), axis=1
        )
    df["Рб.д"] = df[day_cols].apply(
        lambda row: sum(1 for x in row if x == "Д"), axis=1
        )
...
def schedule_to_table(
    ...
    """
    Превращает schedule в таблицу exel.
    """
...
schedule = generate_month_shift_schedule(
    year=YEAR,
    month=MONTH,
    employees=employees_data
)
save_to_excel(schedule, filename="График_работ_2026.xlsx")
